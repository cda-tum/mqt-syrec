...
namespace revkit
{
  ...
  garbagefree_syrec_synthesizer::garbagefree_syrec_synthesizer( circuit& circ, const syrec::program& prog ) : standard_syrec_synthesizer( circ, prog )
  {
    depth = 0;
    dupl_count = 0;
...
//	std::cout << std::endl;
  ...
  bool garbagefree_syrec_synthesizer::on_statement( const syrec::call_statement& statement )
  {
    ...
    else
    {
      ...
      foreach_ ( syrec::variable::ptr var, statement.target()->variables() )
      {
        var_lines().erase( var );
      }
    }
	...
	switch ( exp->op() )
	{
	  case syrec::unary_expression::logical_not: // !
	    ...
	    return syrec::expression::ptr( new syrec::numeric_expression( syrec::number::ptr( new syrec::number( value ) ), 1 ) ); 
          ...
          {
	    std::cerr << boost::format( "Bitwise NOT is undefined for numbers w/o specified bit width: ~%s" )
            % *exp << std::endl;
            assert( false );
	  } break;
          default:
	...
	if ( syrec::numeric_expression* rhs_exp = dynamic_cast<syrec::numeric_expression*>( rhs.get() ) )
	{
	  ...
	  switch ( exp->op() )
	      ...
	      case syrec::binary_expression::frac_divide: // *>    
              {
	        std::cerr << boost::format( "Operator *> is undefined for numbers w/o specified bit width: ( %d *> %d )" )
                % lhs_value % rhs_value << std::endl;
                assert( false );
              } break;
  ...
  unsigned garbagefree_syrec_synthesizer::get_reusable_constant_line( bool value )
  {
    ...
    else if ( !free_const_lines[!value].empty() )
    {
      ...
      foreach_ ( unsigned actrl, actrls )
      {
  ...
  */
  ...
  bool garbagefree_syrec_synthesizer::reverse_modulo( const std::vector<unsigned>& dest, const std::vector<unsigned>& src1, const std::vector<unsigned>& src2 )
  {
    ...
    for ( unsigned i = 0u; i < src1.size(); ++i )
    {
      if ( i > 0 )
      {
	for( int j = int( src1.size() ) - 1; j >= ( src1.size() + 1u - i ); --j )
	{
	  remove_active_control( src2.at( j ) );
	}
	append_not( *( get( boost::vertex_name, cct_man.tree )[cct_man.current].circ ), src2.at( src1.size() - i ) );
	for( int j = int( src1.size() ) - 1; j >= ( src1.size() - i ); --j )
	{
	  add_active_control( src2.at( j ) );
	}
      }
  ...
  void garbagefree_syrec_synthesizer::get_expr_lines( syrec::expression::ptr expression, std::vector<unsigned>& lines, std::list< boost::tuple< bool, bool, unsigned > >& expr_cl )
  {
    ...
    {     
      ...
      else*/ if ( ( expr->op() == syrec::binary_expression::modulo ) || ( expr->op() == syrec::binary_expression::frac_divide ) )
      {
