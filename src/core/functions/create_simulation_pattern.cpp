...
namespace revkit {
    ...
    bool create_simulation_pattern(const pattern& p, const circuit& circ, std::vector<boost::dynamic_bitset<>>& sim, std::map<std::string, boost::dynamic_bitset<>>& init, std::string* error) {
        ...
        foreach_(const pattern::initializer_map::value_type& pa, p.initializers()) {
        ...
        foreach_(const bus_collection::map::value_type& pa, circStates) {
        ...
        foreach_(const std::string& input_name, p.inputs()) {
            ...
            if (busIt != circ.inputbuses().buses().end()) {
                ...
                foreach_(unsigned line, busLines) {
                    usedLines += line;
                    ++numBits;
                }
                vars += input_name;
        ...
        // remove statesignals
        foreach_(const bus_collection::map::value_type& pa, circStates) {
            foreach_(unsigned line, pa.second) {
        ...
        foreach_(const std::string& varName, vars) {
            std::vector<unsigned> vp;
            foreach_(unsigned line, varPlaceIC[varName]) {
        ...
        for (pattern::pattern_vec::const_iterator step = p.patterns().begin(); step != p.patterns().end(); ++step) {
            ...
            for (std::vector<unsigned>::const_iterator it2 = step->begin(); it2 != step->end(); ++it2) {
                ...
                if (input >= (1u << lines.size())) {
                    if (error) {
                        *error = boost::str(boost::format("In step %d: Input %s has not enough bits to represent the given value.") % (std::distance(p.patterns().begin(), step) + 1u) % vars.at(std::distance(step->begin(), it2)));
                    }
                    return false;
                }
