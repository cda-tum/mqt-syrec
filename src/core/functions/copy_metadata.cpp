...
namespace revkit
{
  ...
  void copy_metadata( const circuit& base, circuit& circ, const copy_metadata_settings& settings )
  {
    ...
    {
      foreach_ ( const pair_t& p, base.inputbuses().buses() )
      {
    ...
    {
      foreach_ ( const pair_t& p, base.outputbuses().buses() )
      {
    ...
    {
      foreach_ ( const pair_t& p, base.statesignals().buses() )
      {
    ...
    if ( settings.copy_modules )
    {
      ...
      foreach_ ( const pair2_t& p, base.modules() )
      {
