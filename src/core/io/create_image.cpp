...
namespace revkit {
    ...
    void create_pstricks_settings::draw_targets(std::ostream& os, float x, const std::vector<float>& ys, const boost::any& target_tag) const {
        if (is_type<toffoli_tag>(target_tag)) {
            ...
            foreach_(const float& y, ys) {
                os << boost::format("\\psline[linewidth=%1%](%2%,%3%)(%4%,%5%)\\psline[linewidth=%1%](%2%,%5%)(%4%,%3%)") % line_width % (x - control_radius) % (y - control_radius) % (x + control_radius) % (y + control_radius)
                   << std::endl;
            }
        } else if (is_type<v_tag>(target_tag)) {
            os << boost::format("\\psframe[linewidth=%1%,fillstyle=solid,fillcolor=white](%2%,%3%)(%4%,%5%) \\rput(%6%,%7%){\\sf V}") % line_width % (x - target_radius) % (ys.at(0) - target_radius) % (x + target_radius) % (ys.at(0) + target_radius) % x % ys.at(0)
               << std::endl;
        } else if (is_type<vplus_tag>(target_tag)) {
            os << boost::format("\\psframe[linewidth=%1%,fillstyle=solid,fillcolor=white](%2%,%3%)(%4%,%5%) \\rput(%6%,%7%){\\({\\sf V}^\\dagger\\)}") % line_width % (x - target_radius) % (ys.at(0) - target_radius) % (x + target_radius) % (ys.at(0) + target_radius) % x % ys.at(0)
               << std::endl;
        }
    }
    ...
    void create_tikz_settings::draw_targets(std::ostream& os, float x, const std::vector<float>& ys, const boost::any& target_tag) const {
        if (is_type<toffoli_tag>(target_tag)) {
            ...
            foreach_(const float& y, ys) {
                os << boost::format("\\draw[line width=%1%] (%2%,%3%) -- ++(%5%,%5%) (%2%,%4%) -- ++(%5%,-%5%);") % line_width % (x - control_radius) % (y - control_radius) % (y + control_radius) % (2 * control_radius)
                   << std::endl;
            }
        } else if (is_type<v_tag>(target_tag)) {
    ...
    void create_image(std::ostream& os, const circuit& circ, create_image_settings& settings) {
        ...
        foreach_(const gate& g, circ) {
            ...
            if (!is_peres(g)) {
                ...
                foreach_(unsigned& control_index, controls) {
