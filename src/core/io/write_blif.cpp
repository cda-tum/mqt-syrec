...
namespace revkit {
    ...
    inline const typename boost::tuples::element<N, T>::type& getN(const T& t) {
        return boost::tuples::get<N>(t);
    }

    struct signal_name {
        signal_name(unsigned& tmp_signal, const std::string& tmp_signal_name):
            tmp_signal(tmp_signal),
            tmp_signal_name(tmp_signal_name) {}

        template<typename T>
        std::string operator()(const T& tuple) const {
            if (!boost::get<1>(tuple)) {
    ...
    struct random_access {
        ...
        explicit random_access(const T& container):
            container(container) {}
    ...
    void rephrase_signal_names(const std::vector<std::string>& signals, const bus_collection& bus, std::vector<std::string>& new_signals) {
        for (unsigned i = 0u; i < signals.size(); ++i) {
            new_signals += bus.has_bus(i) ? boost::str(boost::format("%s<%d>") % bus.find_bus(i) % bus.signal_index(i)) : signals.at(i);
        }
    }
    ...
    void write_blif(const circuit& circ, std::ostream& os, const write_blif_settings& settings) {
        ...
        rephrase_signal_names(circ.inputs(), circ.inputbuses(), _inputs);
        rephrase_signal_names(circ.outputs(), circ.outputbuses(), _outputs);
        ...
        std::transform(boost::make_filter_iterator<is_primary>(outputs.begin(), outputs.end()),
                       ...
                       std::ostream_iterator<std::string>(os, "_out "),
                       getN<0, out_tuple>);
        os << std::endl;

        std::transform(inputs.begin(), inputs.end(), signals.begin(), signal_name(tmp_signal, settings.tmp_signal_name));
