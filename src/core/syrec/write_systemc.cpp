...
namespace revkit {
    ...
    std::string systemc_writer::variable_access_to_systemc(const syrec::variable_access& va) {
        std::string va_str = "_";
        va_str += va.var()->name();
        for (std::shared_ptr<syrec::expression> index_expr: va.indexes()) {
            va_str += "[";
    ...
    std::string systemc_writer::statement_to_systemc(const syrec::call_statement& statement, unsigned level) {
        ...
        unsigned    param_no     = 0;
        for (std::string param_str: statement.parameters()) {
    ...
    std::string systemc_writer::module_to_systemc(syrec::module::ptr module) {
        ...
        if (!mod->parameters().empty()) {
            for (syrec::variable::ptr param: mod->parameters()) {
                mod_systemc += TAB;
                mod_systemc += "sc_";
                ...
                for (unsigned i: param->dimensions()) {
                    mod_systemc += "[";
            ...
            for (syrec::variable::ptr param: io_ports) {
                mod_systemc += TAB;
                mod_systemc += "sc_out";
                if (param->bitwidth() <= 64) {
                    mod_systemc += "< sc_uint<";
        ...
        // state-Variablen
        for (syrec::variable::ptr var: mod->variables()) {
        ...
        for (syrec::statement::ptr stat: mod->statements()) {
    ...
    mod_systemc += "\n";*/
        ...
        for (std::list<syrec::call_statement>::const_iterator it = calls.begin(); it != calls.end(); ++it, ++mno) {
            ...
            ctor_str += "_activate);\n";
            unsigned param_no = 0;
            for (std::string param_str: it->parameters()) {
                syrec::variable::ptr mod_param = mod->find_parameter_or_variable(param_str);
      ...
      if(param->type() != syrec::variable::out)
      {
        ...
        if ((signals.size() > 0) || (rev_signals.size() > 0)) {
            ...
            for (unsigned i = 0; i < io_signals.size(); ++i) {
                for (std::string sig: io_signals[i]) {
                    syrec::variable::ptr var = mod->find_parameter_or_variable(sig);
                    mod_systemc += TAB;
                    mod_systemc += "sc_signal< ";
                    if (var->bitwidth() <= 64) {
            ...
            for (unsigned i = 0; i < rev_signals.size(); ++i) {
                for (std::string sig: rev_signals[i]) {
                    syrec::variable::ptr var = mod->find_parameter_or_variable(sig);
                    mod_systemc += TAB;
                    mod_systemc += "sc_signal< ";
                    if (var->bitwidth() <= 64) {
            ...
            for (unsigned i = 0; i < rev_io_signals.size(); ++i) {
                for (std::string sig: rev_io_signals[i]) {
                    syrec::variable::ptr var = mod->find_parameter_or_variable(sig);
                    mod_systemc += TAB;
                    mod_systemc += "sc_signal< ";
                    if (var->bitwidth() <= 64) {
    ...
    void systemc_writer::program_to_systemc(const syrec::program& prog, std::ostream& os) {
        ...
        for (syrec::module::ptr module: prog.modules()) {
    ...
    */
        ...
        for (std::vector<std::string>::const_iterator i = p.inputs().begin(); i != p.inputs().end(); ++i) {
            for (syrec::variable::ptr param: main_mod->parameters()) {
                if (*i == param->name()) {
        ...
        for (std::vector<std::vector<unsigned int>>::const_iterator it = p.patterns().begin(); it != p.patterns().end(); ++it) {
            ...
            for (syrec::variable::ptr param: stimgen_out) {
                stimgen += TAB;
                stimgen += TAB;
                stimgen += "port_";
                stimgen += param->name();
        ...
        stimgen += TAB;
        stimgen += TAB;
        for (syrec::variable::ptr param: stimgen_inout) {
            stimgen += "port_";
            stimgen += param->name();
            if (param->type() == syrec::variable::inout) {
                stimgen += "_in";
        ...
        /*for ( std::map<std::string,unsigned int>::const_iterator i = p.init.begin(); i != p.init.end(); ++i)
    {
      ...
      stimgen += TAB;
        ...
        for (syrec::variable::ptr param: main_io_ports) {
            monitor += TAB;
            monitor += "sc_in";
            if (param->bitwidth() <= 64) {
                monitor += "< sc_uint<";
