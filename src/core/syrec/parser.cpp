...
namespace revkit
{
  ...
  variable_access::ptr parse_variable_access( const ast_variable& ast_var, const module& proc, parser_context& context )
  {
    ...
    foreach_ ( const ast_expression& ast_exp, ast_var.indexes )
    {
  ...
  struct expression_visitor : public boost::static_visitor<expression*>
  {
	...
	op = unary_expression::bitwise_not;
      ...
      {
	if ( ( ( op == unary_expression::bitwise_not ) && ( sub_expr->op() == unary_expression::bitwise_not ) )
	     || ( ( sub_expr->expr()->bitwidth() == 1u ) 
	          && ( ( op == unary_expression::bitwise_not ) || ( op == unary_expression::logical_not ) ) 
		  && ( ( sub_expr->op() == unary_expression::bitwise_not ) || ( sub_expr->op() == unary_expression::logical_not ) ) ) )
	{
  ...
  struct statement_visitor : public boost::static_visitor<statement*>
  {
    ...
    statement* operator()( const ast_swap_statement& ast_swap_stat ) const
    {
      ...
      {
        std::cerr << boost::format( "Different bit-widths in <=> statement: %s (%d), %s (%d)" )
          % va1->var()->name() % va1->bitwidth() 
          % va2->var()->name() % va2->bitwidth() << std::endl;
        assert( false );
        return 0;
      }
    ...
    statement* operator()( const ast_if_statement& ast_if_stat ) const
    {
      ...
      foreach_ ( const ast_statement& ast_stat, ast_if_stat.if_statement )
      {
      ...
      foreach_ ( const ast_statement& ast_stat, ast_if_stat.else_statement )
      {
        statement::ptr stat = parse_statement( ast_stat, prog, proc, context );
        if ( !stat ) return 0;
    ...
    statement* operator()( const ast_for_statement& ast_for_stat ) const
    {
      ...
      foreach_ ( const ast_statement& ast_stat, ast_for_stat.do_statement )
      {
        statement::ptr stat = parse_statement( ast_stat, prog, proc, context );
        if ( !stat ) return 0;
    ...
    statement* operator()( const ast_call_statement& ast_call_stat ) const
    {
      ...
      foreach_ ( const std::string& parameter, parameters )
      {
  ...
  bool parse_module( syrec::module& proc, const ast_module& ast_proc, const syrec::program& prog, parser_context& context )
  {
    ...
    foreach_ ( const ast_parameter& ast_param, bf::at_c<1>( ast_proc ) )
    {
    ...
    foreach_ ( const ast_variable_declarations& ast_decls, bf::at_c<2>( ast_proc ) )
    {
      unsigned type = parse_variable_type( bf::at_c<0>( ast_decls ) );
      
      foreach_ ( const ast_variable_declaration& ast_decl, bf::at_c<1>( ast_decls ) )
      {
    ...
    foreach_ ( const ast_statement& ast_stat, bf::at_c<3>( ast_proc ) )
    {
      statement::ptr stat = parse_statement( ast_stat, prog, proc, context );
  ...
  bool read_program_from_string( syrec::program& prog, const std::string& content, const read_program_settings& settings, std::string* error )
  {
    ...
    // Modules
    foreach_ ( const ast_module& ast_proc, ast_prog )
    {
