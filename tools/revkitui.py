...
class SyReCEditor:
    ...
    def setupActions( self ):
        ...
        self.buildAction = QAction( QIcon.fromTheme( 'run-build' ), '&Build...', self.parent )
        self.simAction = QAction( QIcon.fromTheme( 'run-sim' ), '&Sim...', self.parent )
        self.statAction = QAction( QIcon.fromTheme( 'run-stats' ), '&Stats...', self.parent )

        self.parent.connect( self.openAction, SIGNAL( 'triggered()' ), self.open)
        self.parent.connect( self.buildAction, SIGNAL( 'triggered()' ), self.build)
        self.parent.connect( self.simAction, SIGNAL( 'triggered()' ), self.sim)
        self.parent.connect( self.statAction, SIGNAL( 'triggered()' ), self.stat)
    ...
    def build( self ):
        ...
        r = read_programi( prog, "/tmp/out.src" )
...
#        print(type(prog))
        ...
        print (circ_n.num_gates)
        
        #print (circ.constants)
        print (circ_n.lines)
        print (circ_n.inputs)
        print (circ_n.constants)
        
        print (circ_n.garbage)
        #print (circ_n.filter)
        #ss = propertiesi()
        #st = propertiesi()
        #my_bitset = bitseti(circ_n.lines, 15)
        #print(str(my_bitset))
        #my_inp_bitset = bitset(circ.lines, i) 
        #my_out_bitset = bitseti(circ_n.lines, 9)
        #py_simple_simulationi(my_out_bitset, circ_n, my_bitset,ss,st)
        #print(str(my_out_bitset)) 
   
        #if(circ.inputs==circ_n.inputs):
        #    print("ji") 
       
        if self.build_successful is not None:
            self.build_successful( circ_n )
    ...
    def stat(self):
        ...
        lines = circ.lines
        #qc=py_costs(circ,quantum_costs())
        qc=0
        tc=0
        #for g in circ.gates:
        ...
        #tc = py_costs(circ,transistor_costs())
        ...
        #msg.setText("Circuit Statistics")
...
#--------------------------------------------------------------------------------
        ...
        table 	= QTableWidget()
        #tableItem 	= QTableWidgetItem('red')
        
        # initiate table
        table.setWindowTitle("SyReC Simulation")
        table.resize(800, 250)
        #table.setRowCount(1)
        #table.setColumnCount(2)

        table.setRowCount(input_list_len + 2)
        table.setColumnCount(2*num_inputs)

        #for i in range(num_input)
        #    table.setItem(0,0, QTableWidgetItem("Inputs"))
        #table.setItem(0,1, QTableWidgetItem("Outputs"))
        ...
        table.setSpan(0, 0, 1, num_inputs) 
        header1 = QTableWidgetItem("INPUTS")
        header1.setTextAlignment(QtCore.Qt.AlignCenter)  
        table.setItem(0, 0, header1) 

        table.setSpan(0, num_inputs, 1, num_inputs) 
        header2 = QTableWidgetItem("OUTPUTS")
        header2.setTextAlignment(QtCore.Qt.AlignCenter)     
        table.setItem(0, num_inputs, header2) 


        table.horizontalHeader().setVisible(False)
        table.verticalHeader().setVisible(False)
        #table.setHorizontalHeaderItem(0,tableItem)
        #table.setHorizontalHeaderLabels(QString(inp_string).split(";"))
        ...
        for i in range(num_inputs):
            ...
            table.setItem(1,i, QTableWidgetItem(input_signal))

            output_signal=QTableWidgetItem(circ.outputs[i])
            output_signal.setTextAlignment(QtCore.Qt.AlignCenter)
            table.setItem(1,i+num_inputs, QTableWidgetItem(output_signal))
        # set data
        for i in range(input_list_len):
            for j in range(num_inputs):
                ...
                table.setItem(i+2,j, QTableWidgetItem(input_cell))

                output_cell=QTableWidgetItem(final_out[i][j])
                output_cell.setTextAlignment(QtCore.Qt.AlignCenter)
                table.setItem(i+2,j+num_inputs, QTableWidgetItem(output_cell))
        
        # show table
        table.show()
        table.exec_()
        #for c in combinations :
        #    print str(c[0].to_ulong()) + " " + str(c[0]) + " " + str(c[0].flip)  + " " + str(c[1].to_ulong()) + " " + str(c[1])    


        #combinations_x = sorted(combinations, key = lambda v: v[0].to_ulong() << left_inc)

        #for c in combinations_x :
        #    print str(c[0].to_ulong()) + " " + str(c[0])  + " " + str(c[1].to_ulong()) + " " + str(c[1]) 
